...
const GuardCore = require('./GuardCore');

class GuardRoles extends GuardCore {
  /**
   ...
   */
  makeRoles(roles, options = { json: true }) {
    let _guard = this;
  ...
  /**
    ...
    *
   * @example
   * guard.addPermsToRole('admin','view','blogs');
   ...
   */
  async addPermsToRole(role, actions, resources) {
    let guard = this;
    return guard
      .createPerms(resources, actions, { all: true })
      .then((perms) => {
        return guard
          .makeRole(role)
          .then(({ role }) => {
   ...
   *
   * @example
   * guard.rmPermsFromRole('admin','view','blogs');
   ...
   */
  async rmPermsFromRole(role, actions, resources) {
    let guard = this;
